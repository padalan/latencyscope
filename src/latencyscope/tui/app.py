...
from latencyscope.tui.interrupts import InterruptStat, InterruptWatcher
from latencyscope.tui.jitter import JitterResult, NetworkJitter
from latencyscope.tui.radar import ProcessInfo, Radar
...
class LatencyScopeTUI:
    ...
    def make_layout(self) -> Layout:
        ...
        layout["main"].split_row(Layout(name="canary"), Layout(name="radar"))
        layout["footer"].split_row(Layout(name="interrupts"), Layout(name="network"))
        return layout
    ...
    def generate_radar_panel(self, procs: list[ProcessInfo]) -> Panel:
        ...
        for p in procs:
            table.add_row(str(p.pid), p.name, f"{p.cpu_percent:.1f}%")
